...
namespace SO_Architecture.References
{
    public abstract class BaseReference<T>
    {
        [SerializeField] private ReferenceType variableType = ReferenceType.Constant;

        [SerializeField, ShowIf("variableType", ReferenceType.Constant)]
        private T constantValue = default;

        [SerializeReference, ShowIf("variableType", ReferenceType.ScriptableObject)]
        private Variable<T> scriptableObject = null;

        [SerializeReference, ShowIf("variableType", ReferenceType.Instance)]
        private VariableInstance<T> instance = null;
